...
const double DurThreshold = 500;
const double StepSize = 0.05;
const double TargetFrequency = .2;
const double DistThreshold = 0.05;
...
double BT = 1.0;	// Default Bias Time Constant
double WT = 1.0;	// Default Weight Time Constant

// Tau Sweep parameters
const double HPtaustart = .5;
const double HPtaustop = 100;
const double HPtaustep = .5;
const int Circuits = 50; //Number of circuits to evolve (runs to complete) at each value of HP tau
...
// 	NO LONGER INCLUDING A PHASE TO MAXIMIZE CUMULATIVE RATE OF CHANGE; THIS WOULD BIAS RESULTS AWAY FROM HP-ENABLED OSCILLATION
...
// 	// Run the circuit for an initial transient (Fitness is not evaluated)
// 	for (double time = StepSize; time <= TransientDuration; time += StepSize) {
// 		Agent.EulerStep(StepSize);
// 	}

// 	// Run the circuit to calculate how many neurons are oscillating (according to Beer 2006)
// 	CumRateChange.FillContents(0.0);
// 	for (double time = StepSize; time <= OscillationDuration; time += StepSize) {
// 		for (int i = 1; i <= N; i += 1) {
...
// 	double perf = 0.0;
// 	for (int i = 1; i <= N; i += 1) {
// 		perf += CumRateChange[i]/OscillationDuration;
// 	}
// 	perf /= 3;
// 	return perf;
// }
...
int main (int argc, const char* argv[]) 
{
	...
	for (double HPtau = HPtaustart;HPtau<=HPtaustop;HPtau+=HPtaustep){
		...
		for (int c=1;c<=Circuits;c++){
			...
			if(doBehavior){Behavior(genotype);}
