...
//
// Created by RDB
// Modified to Include HP by LJS & EJI
// ************************************************************
...
TVector<int> CTRNN::TestOscillation(int n, double StepSize, double TransientDuration, double RunDuration, RandomState &rs)
{
    int maxdimact = 0;
    int ICdependentindicator = 0;
    // Initialize the state between [-16,16] at random
    RandomizeCircuitState(-16.0, 16.0, rs);
    ...
    for (double time = StepSize; time <= RunDuration; time += StepSize) {
        for (int i = 1; i <= n; i += 1) {
          pastNeuronOutput[i] = NeuronOutput(i);
        }
        EulerStep(StepSize);
        for (int i = 1; i <= n; i += 1) {
          activity[i] += fabs(NeuronOutput(i) - pastNeuronOutput[i]);
        }
    }
